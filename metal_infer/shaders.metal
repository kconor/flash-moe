...
// ============================================================================
// Kernel 1h: BF16 matvec (no quantization — raw bf16 weights)
// ============================================================================
// Weights are raw BF16 (uint16), no scales/biases. Just convert and multiply.
// Buffer layout differs from quantized kernels:
//   buffer(0) = W_bf16 [out_dim, in_dim] as uint16
//   buffer(1) = unused (pass any valid buffer)
//   buffer(2) = unused (pass any valid buffer)
//   buffer(3) = x [in_dim] float
//   buffer(4) = out [out_dim] float

kernel void matvec_bf16(
    device const uint16_t* W_bf16     [[buffer(0)]],  // [out_dim, in_dim] bf16
    device const uint16_t* _unused1   [[buffer(1)]],  // unused (scales placeholder)
    device const uint16_t* _unused2   [[buffer(2)]],  // unused (biases placeholder)
    device const float*    x          [[buffer(3)]],  // [in_dim]
    device float*          out        [[buffer(4)]],  // [out_dim]
    constant uint&         out_dim    [[buffer(5)]],
    constant uint&         in_dim     [[buffer(6)]],
    constant uint&         _unused_gs [[buffer(7)]],  // unused (group_size placeholder)
    uint tgid       [[threadgroup_position_in_grid]],
    uint lid        [[thread_position_in_threadgroup]],
    uint simd_lane  [[thread_index_in_simdgroup]],
    uint simd_group [[simdgroup_index_in_threadgroup]]
...
// BF16 variant for in_dim > 4096 (no shared memory cache)
kernel void matvec_bf16_fast(
    device const uint16_t* W_bf16     [[buffer(0)]],
    device const uint16_t* _unused1   [[buffer(1)]],
    device const uint16_t* _unused2   [[buffer(2)]],
    device const float*    x          [[buffer(3)]],
    device float*          out        [[buffer(4)]],
    constant uint&         out_dim    [[buffer(5)]],
    constant uint&         in_dim     [[buffer(6)]],
    constant uint&         _unused_gs [[buffer(7)]],
    uint tgid [[threadgroup_position_in_grid]],
    uint lid  [[thread_position_in_threadgroup]],
    uint tg_size [[threads_per_threadgroup]]
) {
    ...
    device const uint16_t* w_row = W_bf16 + tgid * in_dim;
    float acc = 0.0f;
